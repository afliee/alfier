...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('music_forward')
        .setDescription('Forward seconds in a song specified')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (timeFormatted && queue.duration > timeFormatted / 1000) {
                await queue.seek(queue.currentTime + timeFormatted / 1000);
                embed
                    .setColor('Green')
                    .setDescription(`⏭️ | Forward the song for ${time}`);
                return interaction.reply({
                    embeds: [embed],
                });
